...
class CONCHEmbedder(CaptionEmbedder):
    """
    for 1024x1024 images and above, CONCH gives us a grid of nxd embeddings
    instead of passing it directly to the model, we pass it through a transformer block first
    this will add positional information and allow interactions between the embeddings
    """

    def __init__(self, in_channels, hidden_size, uncond_prob, act_layer=nn.GELU(approximate='tanh'), token_num=120, num_layers=4, num_heads=8):
        super().__init__(in_channels, hidden_size, uncond_prob, act_layer, token_num)
    ...
    def forward(self, caption, train, force_drop_ids=None):
        ...
        caption = self.encoder(caption.squeeze()).unsqueeze(1)
        caption = self.y_proj(caption)
        caption = self.layer_norm(caption)

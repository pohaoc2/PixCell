...
def pixcell_get_2d_sincos_pos_embed(
    ...
    pos_embed = pixcell_get_2d_sincos_pos_embed_from_grid(
        embed_dim, grid, phase=phase, output_type=output_type
    )
...
def pixcell_get_2d_sincos_pos_embed_from_grid(embed_dim, grid, phase=0, output_type="pt"):
    ...
    emb_h = pixcell_get_1d_sincos_pos_embed_from_grid(
        embed_dim // 2, grid[0], phase=phase, output_type=output_type
    )
    emb_w = pixcell_get_1d_sincos_pos_embed_from_grid(
        embed_dim // 2, grid[1], phase=phase, output_type=output_type
    )
...
class PixCellControlNet(ModelMixin, ConfigMixin):
    ...
    def from_pretrained_base_model(cls, base_model_path, **controlnet_kwargs):
        ...
        if not_found_keys:
            print(
                f"Note: {len(not_found_keys)} parameters not found in base model (expected for ControlNet-specific layers)"
            )

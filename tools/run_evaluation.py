"""
tools/run_evaluation.py — Stage 3 batch inference + full visualizations.

Output per tile:
    {output_dir}/{tile_id}/{paired,unpaired}/
        generated_he.png    generated H&E image
        overview.png        TME input channels → generated H&E
        residuals.png       per-group conditioning latent residuals
    ...
    *,
    ...
    if uni_extractor is not None:
        ...
        if paired_uni is not None:
            ...
            sim = _cosine_sim(gen_feat, exp_feat)
    save_overview_figure(
        ctrl_full=ctrl_full,
...
def main():
    ...
    # Collect tile IDs from cell_masks (or legacy cell_mask) directory
    mask_dir = EXP_CH_DIR / "cell_masks"
    if not mask_dir.is_dir():
        mask_dir = EXP_CH_DIR / "cell_mask"

...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # slurm time limit (min) will be passed as an argument 
...
def parse_args():
    ...
    parser.add_argument('--batch-size', type=int, default=None, help='overwrite the batch size in config')
    parser.add_argument(
        "--report_to",
        type=str,
        default="wandb",
    ...
    parser.add_argument(
        ...
        type=str,
        default="pan-cancer-diffusion",
        help=(
            "The `project_name` argument passed to Accelerator.init_trackers for"
            " more information see https://huggingface.co/docs/accelerate/v0.17.0/en/package_reference/accelerator#accelerate.Accelerator"
        ),
    )
    parser.add_argument("--slurm_time_limit", type=int, default=1435,
         help="time limit for slurm job in minutes, ~5 min less than the actual limit")
...
if __name__ == '__main__':
    ...
    model_kwargs = {"pe_interpolation": config.pe_interpolation, "config": config,
                    ...
                    "add_pos_embed_to_cond": getattr(config, 'add_pos_embed_to_cond', False), **config.get('model_kwargs', {})}
    ...
    if config.load_from is not None:
        missing, unexpected = load_checkpoint(
            config.load_from, model, load_ema=config.get('load_ema', False), max_length=max_length, ignore_keys=config.get('ignore_keys', []))

...
def _resume_from_checkpoint(
    config, model, controlnet, model_ema, optimizer, lr_scheduler, max_length, logger
):
...
def initialize_models(config, accelerator, logger):
    ...
    # ===================================================================
    vae = AutoencoderKL.from_pretrained(config.vae_pretrained, torch_dtype=torch.float16).to(
        accelerator.device
    )
    ...
    base_stats = load_model_weights_flexible(
        base_model,
        load_file,
        remap_pixcell_safetensors=True,
        verbose=True,
    )
    logger.warning(f"Base model - loaded tensors: {base_stats['loaded']}")
    logger.warning(f"Base model - missing keys: {len(base_stats['missing'])}")
    logger.warning(f"Base model - unexpected keys: {len(base_stats['unexpected'])}")
    ...
    logger.info(
        f"Trainable parameters: {sum(p.numel() for p in controlnet.parameters() if p.requires_grad):,}"
    )
    ...
    if config.get("controlnet_load_from", None) is not None:
        ...
        stats = load_controlnet_weights_flexible(controlnet, load_file, verbose=True)
        logger.warning(f"ControlNet - loaded tensors: {stats['loaded']}")
        logger.warning(f"ControlNet - missing keys: {len(stats['missing'])}")
        logger.warning(f"ControlNet - unexpected keys: {len(stats['unexpected'])}")
    ...
    ema_update(model_ema, controlnet, 0.0)
...
def train_controlnet(models_dict):
    ...
    if config.get("debug_nan", False):
        ...
        logger.info(
            "NaN debugger registered for ControlNet. Start to detect overflow during training."
        )
...
def training_losses_controlnet(
    diffusion, controlnet, base_model, x_start, timesteps, model_kwargs=None, config=None
):
    """
    ...
    map_tensor = torch.tensor(
        diffusion.timestep_map, device=timesteps.device, dtype=timesteps.dtype
    )
...
def main():
    ...
    base_model, controlnet, model_ema, optimizer, train_dataloader, lr_scheduler = (
        accelerator.prepare(
            base_model, controlnet, model_ema, optimizer, train_dataloader, lr_scheduler
        )
    )
    ...
    scheduler = DPMSolverMultistepScheduler.from_pretrained(
        "StonyBrook-CVLab/PixCell-256", subfolder="scheduler"
    )
    latent_shape = (
        1,
        16,

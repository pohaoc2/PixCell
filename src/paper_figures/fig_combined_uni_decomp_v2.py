...
# Panel F is trimmed to these three attributes for the narrower right column.
F_SWEEP_ATTRS = ["eccentricity_mean", "nuclear_area_mean", "texture_e_contrast"]
...
def build_left_column_figure(
    *,
    ...
    # Panel letters share the left edge (same column).
    ...
    _fig_letter(fig, fig_w=fig_w, fig_h=fig_h, x_in=M_L, y_in=c_top + 0.16, letter="C")
...
def _render_right_column_DEF() -> tuple[np.ndarray, int]:
    """D (probe ΔR²) | E (specificity) over F (3-attribute sweep grid)."""
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            a4_figures.FONT_AXIS_LABEL = 13
            a4_figures.FONT_TICK = 11
            a4_figures.FONT_INAXES = 11
            a4_figures.FONT_LEGEND = 11
...
def build_uni_decomp_v2_figure() -> plt.Figure:
    ...
    c_heatmap_bottom_y = round(
        getattr(left_fig, "_pixcell_c_heatmap_bottom_frac") * left_fig.get_figheight() * _LEFT_DPI
    )
    ...
    right_canvas = np.full((height, right.shape[1], 3), 255, dtype=np.uint8)
    f_bottom_y = min(height, c_heatmap_bottom_y)
    if 0 < f_start_y < right.shape[0]:
        top = right[:f_start_y]
        bottom = right[f_start_y:]
        right_canvas[:top.shape[0], :top.shape[1]] = top
        y0 = max(0, f_bottom_y - bottom.shape[0])
        y1 = min(f_bottom_y, y0 + bottom.shape[0])
        right_canvas[y0:y1, :bottom.shape[1]] = bottom[:y1 - y0]
    else:
    ...
    gutter = np.full((height, gap_px, 3), 255, dtype=np.uint8)

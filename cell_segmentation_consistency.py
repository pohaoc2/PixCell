...
Uses Cellpose (with SAM support) for segmentation
Install: pip install git+https://www.github.com/mouseland/cellpose.git
"""
import torch
...
class CellSegmentationConsistency(nn.Module):
    """
    Cell segmentation consistency checker using Cellpose
    ...
    Args:
        model_type: Cellpose model type ('cyto', 'nuclei', 'cyto2', 'cyto3')
    ...
    def __init__(
        ...
        image_size=256,
        use_gpu=True,
        diameter=30,  # Typical cell diameter for 20x histopathology
    ):
        super().__init__()
        ...
        print(f"Initializing Cellpose segmentation with model: {model_type}")
    
    @property
    def model(self):
        """Lazy load Cellpose model to save memory"""
        if self._model is None:
            self._model = self._load_cellpose_model()
        return self._model
    ...
    def _load_cellpose_model(self):
        ...
        try:
            ...
            print(f"Loading Cellpose model: {self.model_type}")
            ...
            print(f"✓ Cellpose model loaded successfully")
            ...
            print("  Install with: pip install git+https://www.github.com/mouseland/cellpose.git")
            print("  Using simplified segmentation model as fallback")
            return self._create_simple_segmentation_model()
        except Exception as e:
            print(f"⚠ Error loading Cellpose: {e}")
            print("  Using simplified segmentation model as fallback")
            return self._create_simple_segmentation_model()
    ...
    def segment_image(self, image):
        """
        Segment cells in an image using Cellpose
        ...
        # Convert to numpy for Cellpose (expects [0, 255])
        images_np = (image.cpu().numpy() * 255).astype(np.uint8)
        images_np = images_np.transpose(0, 2, 3, 1)  # (B, H, W, C)
        
        masks = []
        
        # Check if using Cellpose or fallback
        if hasattr(self.model, 'eval'):  # Cellpose model
            for img_np in images_np:
                try:
                    ...
                    mask, flows, styles, diams = self.model.eval(
                        ...
                        channels=[0, 0],  # Grayscale or first channel
                        flow_threshold=0.4,
                        cellprob_threshold=0.0,
                    )
                    
                    # Convert to binary (any cell = 1, background = 0)
                    binary_mask = (mask > 0).astype(np.float32)
                    masks.append(binary_mask)
                    
                except Exception as e:
                    print(f"⚠ Cellpose segmentation failed: {e}, using zeros")
                    masks.append(np.zeros((img_np.shape[0], img_np.shape[1]), dtype=np.float32))
        
        else:  # Fallback U-Net model
            with torch.no_grad():
        ...
        masks = torch.from_numpy(masks).unsqueeze(1).to(image.device)  # (B, 1, H, W)
...
if __name__ == "__main__":
    print("Testing Cell Segmentation Consistency Module with Cellpose...")
    print("=" * 70)
    
    # Create consistency checker
    consistency = CellSegmentationConsistency(
        model_type='cyto2',  # Good for general cell segmentation
        device='cuda' if torch.cuda.is_available() else 'cpu',
        image_size=256,
        diameter=30,  # Typical for histopathology at 20x
    )
    ...
    print("\n1. Segmenting generated images with Cellpose...")

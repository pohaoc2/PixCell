...
def run_validation(
    ...
    for sim_id in sim_ids:
        ...
        with torch.no_grad():
            gen_img = vae.decode((denoised.float() / vae_scale) + vae_shift, return_dict=False)[0]
...
def main():
    ...
    parser.add_argument(
        "--exp-feat", required=True, help="Directory of *_uni.npy precomputed exp target features"
    )
    parser.add_argument("--controlnet-ckpt", required=True)
    parser.add_argument("--tme-ckpt", required=True)
    parser.add_argument(
        "--uni-model", default="./pretrained_models/uni-2h", help="Path to UNI-2h model directory"
    )
    parser.add_argument(
        "--reference-uni",
        default=None,
    ...
    parser.add_argument(
        "--output-dir", default=None, help="Save generated H&E images here (optional)"
    )

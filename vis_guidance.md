...
- **Equal gaps (and square cells) come from absolute inches, not GridSpec fractions.** When a grid of equal-status cells (image tiles *or* sub-panels) must read with an equal gap horizontally and vertically, lay it out in inch-space — one square cell size + one gap constant used for *both* axes — and place each cell with `fig.add_axes([x/fig_w, y/fig_h, cell/fig_w, cell/fig_h])`. Do **not** use `GridSpec(wspace=, hspace=)`: those are fractions of the *average cell width/height*, so equal numbers give *unequal* physical gaps the moment the figure's width ≠ height, and they drift whenever the figure is resized. The same inch-space trick keeps heatmap cells square (size the host axes to `n_cols·CELL × n_rows·CELL` so `aspect="auto"` fills it with square cells). (Reference: `fig_uni_tme_decomposition.py` — panel A's 2×3 grid placed cell-by-cell in inches, panel C's square-celled heatmap; `fig_channel_ablation_strip.py` `CELL_IN`/`GAP_IN`.)

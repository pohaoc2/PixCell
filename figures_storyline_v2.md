...
- **Deliverable.**
  ...
  - (C) Per-group effect-size heatmaps:
    - **CS:** ΔPQ +0.20 paired / +0.15 unpaired; ΔDICE +0.29 paired / +0.27 unpaired.
    - **NU:** ΔPQ +0.26 paired / +0.19 unpaired; ΔDICE +0.28 paired / +0.27 unpaired.
    - **CT:** small everywhere (ΔFUD ≈ −0.9; near-zero on PQ/DICE).
    - **VA:** ≈0 on every metric in both regimes.
    - **FUD direction flip:** CS −6.48 paired vs +5.39 unpaired; NU −2.29 paired vs +10.31 unpaired (dense conditioning over-constrains the generator without UNI to anchor style).
...
- **Deliverable.**
  ...
  - **UNI effect** dominates FUD/LPIPS/HED (style/realism).
  - **TME effect** dominates PQ/DICE (nuclear geometry).
  - **FUD interaction ≈ +70** → UNI and TME are partially redundant for realism (either alone recovers most of the gain).
  - **PQ/DICE interaction ≈ 0** → TME's segmentation contribution does *not* depend on UNI; validates TME-only inference for cell-counting tasks.
  - Unconditional baseline collapses to texture noise → all measured effects are real signal.
...
The current heatmap buries the +70 FUD interaction and the ≈0 PQ/DICE interaction. Adding a single annotated arrow per row ("redundant for realism" / "independent for geometry") turns a numerical table into the figure's takeaway. No new data.
...
Fig 3 shows VA ≈ 0 effect across all metrics in both regimes. Carrying it through Fig 4/5 dilutes the per-channel claims. Two options:
- (a) Fold VA into Fig 1B/3 as an explicit negative result ("the channel we expected to matter most for tissue architecture does not — H&E already encodes vasculature geometry"). This is consistent with high R² for cancer/healthy density in Fig 1B.
...
Currently Fig 3's regime-flip on FUD (CS −6.48 paired vs +5.39 unpaired) reads as a surprise. Fig 4's "UNI anchors realism" makes this expected. Reordering the prose so the FUD flip in Fig 3 is framed as a prediction of Fig 4 (cite-forward) — or just stating it parenthetically — converts a confusing regime-dependence into evidence for the dual-conditioning design.
